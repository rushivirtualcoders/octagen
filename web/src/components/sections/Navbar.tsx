import { ASSETS, CLAIM, DISTRIBUTOR, DISTRIBUTOR_TAGLINE, NAV_LINKS } from '../../lib/constants'
...
function Logo() {
  return (
    <a href="#top" className="flex items-center gap-3 sm:gap-4" aria-label={`${DISTRIBUTOR} home`}>
      ...
      <img
        src={ASSETS.liquiMolyLogo}
        alt="LIQUI MOLY"
        className="hidden h-7 w-auto opacity-90 sm:block"
      />
      <span className="tech-label hidden text-muted lg:inline">{DISTRIBUTOR_TAGLINE}</span>
    </a>
  )
}
    ...
    >
      <nav className="mx-auto flex w-full max-w-[1400px] items-center justify-between px-6 lg:px-10">
        ...
        <ul className="hidden items-center gap-7 xl:flex">
          {NAV_LINKS.map((link) => (
            <li key={link.href}>
              <a
                href={link.href}
                className="tech-label text-[#0B1215] transition-colors duration-300 hover:text-lm-orange"
              >
                {link.label}
              </a>
            </li>
        ...
        <div className="hidden items-center gap-4 lg:flex">
          <span className="tech-label hidden text-lm-red xl:inline">{CLAIM}</span>
          <a
            href="#guidance"
              ...
              {NAV_LINKS.map((link, i) => (
                ...
                >
                  <a
                    ...
                    className="font-display block py-2 text-4xl font-extrabold tracking-tight text-[#0B1215] uppercase sm:text-5xl"
                  >
                    {link.label}
                  </a>
                </motion.li>
